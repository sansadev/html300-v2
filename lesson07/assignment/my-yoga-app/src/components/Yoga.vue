<template>

<div class="grid-container">
    <div class="header">
         <header>
           <div class="logo">
           <img src="./images/yoga-chakra.jpg" id ="yoga-chakra" alt="man sitting in ayogic posture">
           </div>
           <h1>Yoga</h1>
        </header>
    </div>
   <div class="nav-bar">
   <nav>
     <ul class="nav-links">
      <li v-for="item in items" v-bind:key="item"><a v-bind:href="item.path">{{item.name}}</a></li>
    </ul>
   </nav>
    </div>
   <div>

   <main class="main">
      <div class="container">
        <h2>LEARN ABOUT YOGA</h2>
        <h2>LEARN ABOUT YOGA</h2>
           <p class="text-danger"><a href="#" data-toggle="tooltip" title="In this section you learn about yoga!!">Yoga for beginners</a></p>
           <p>Deepen your understanding of yoga by exploring the philosophical aspects of this ancient practice. Learn about the history of yoga, get an overview of the major yogic texts,
             and learn more about the chakras, the body’s energy centers and their relationship to health and wellness. We’ve also included a section on using asanas to treat common ailments
             and conditions as well as a compressive list of our articles on yoga.</p>
            <div id="accordion" class="mb-5">
            <div class="card">
                      <div class="card-header bg-warning" v-for="card in cards" v-bind:key="card" >
                        <a class="card-link" data-toogle="collapse"  v-bind:href="card.name" v-on:click="card.open = !card.open">
                          {{ card.title }}
                        </a>
                      </div>
                      <div v-bind:id="card.id" class="collapse" data-parent="#accordion">

                        <div class="accord" v-if="card.open">
                        {{ card.body }}
                        </div>
                    </div>
                  </div>
          </div>
       </div>
 </main>
</div>
    <div class="footer">
        <p class="footer-text">© Copyright 2020 Swati. All rights reserved.</p>
    </div>

   </div>
</template>
<script>
export default {
ready () {
$(document).ready(function(){
$('[data-toggle="tooltip"]').tooltip();
})
},

  name: 'About',
  data() {
  return {

  items: [{"name":"Home",
            "path":"/"},
          {"name":"About",
            "path":"#/about"},
          {"name":"Yoga",
             "path":"#/yoga" },
          {"name":"Pranayama",
             "path":"#/pranayama"}
             ],

     cards: [
                   {
                    "id": "collapseOne",
                    "name":"#collapseOne",
                    "title":"Yoga Philosophy",
                   "body":"lorem ipsum",
                    "open": false
                    },
                    {
                    "id": "collapseTwo",
                    "name":"#collapseTwo",
                    "title": "Yoga Therapy",
                    "body":  "lorem ipsum",
                    "open": false
                    },
                    {
                    "id": "collapseThree",
                    "name":"#collapseThree",
                    "title": "Energy Anatomy",
                    "body": "lorem ipsum",
                    "open": false
                    }
              ]

  }
  }

  }




</script>
